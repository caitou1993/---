...
(function() {
    // if (getQueryString('from') === 'lsk' && getQueryString('x-token')) {
    //     localStorage['x-token'] = decrypto(getQueryString('x-token'));
    // } else {
    //     localStorage['x-token'] ? localStorage.removeItem('x-token') : '';
    // }
    ...
    getCompanyInfo({
        ...
        // 添加蕾丝控跳转至商家网店的收藏星星功能 @author lyb 2017-11-28 10:25:06
        // 初始化浏览次数值
        c('#viewCount').innerHTML = res.data.viewCount;
        c('#view2Count').innerHTML = res.data.viewCount;
        // 根据是否来自蕾丝控判断是否显示收藏星星
        var collectStar = c('#collectStar');
        var collectWrapper = c('#collectWrapper'); // 带收藏
        var starWrapper = c('#starWrapper'); // 不带收藏

        collectWrapper.style.display = 'block';
        (res.data.favoriteSatus) ? (collectStar.className = 'iconfont icon-star active') : ('iconfont icon-star');
        collectStar.onclick = function () {
            ...
            favoriteBus({
                ...
                if (res.code === 0) {
                    ...
                    if (res.message === '收藏成功') {
                        that.className = 'iconfont icon-star active';

                    } else {
                        that.className = 'iconfont icon-star';
                    }
                }
            }, function (res) {
                console.log(res);
        ...
        // }
        // end @author lyb 2017-11-28 10:25:13
    ...
    function bindClick(selector) {
        ...
        function jump() {
            ...
            // location.href = `./patterns_detail.html?companyId=${companyId}&dataId=${dataId}`;
            location.href = '../pattern_detail.html?companyId=' + companyId + '&dataId=' + dataId;
        }
    }
